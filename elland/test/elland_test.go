...
func TestPythonPackagesVersions(t *testing.T) {
  ...
  // Build image
	...
	// Make sure Python version is: 3.9.13
	pythonOpts := &docker.RunOptions{Command: []string{"python", "--version"}}
	pythonOutput := docker.Run(t, *imgName, pythonOpts)
	assert.Equal(t, "3.9.13", pythonOutput)

	// Make sure PyTorch version is: 1.13.1
  torchOpts := &docker.RunOptions{Command: []string{"python", "-c", "import torch; torch.__version__"}}
  torchOutput := docker.Run(t, *imgName, torchOpts)
  assert.Equal(t, "1.13.1", torchOutput)

	// Make sure PyTorch Lightning version is: 1.7.7
  lightningOpts := &docker.RunOptions{Command: []string{"python", "-c", "import pytorch_lightning; pytorch_lightning.__version__"}}
